    ...
    static int solve(CACHE& cache, State& state, const int maxLeft) {
        ...
        for (int i = 0; i < N; i++) {
            ...
            for (int col = 0; col < 5; col++) {
                ...
                newState.board.emplace(newState.board.begin() + i + 1,
                                       newState.board[i].first, 1);
        ...
        for (int i = 0; i < N; i++) {
            ...
            if (!(state.hand[col] < rem || ans <= rem)) {
                ...
                newState.board.erase(newState.board.begin() + l + 1,
                                     newState.board.begin() + r);
